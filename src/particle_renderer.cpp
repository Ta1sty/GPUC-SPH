...
void ParticleRenderer2D::createPipelines() {
    ...
    backgroundPipeline = pipelines[0];
    particlePipeline = pipelines[1];
}
...
void ParticleRenderer2D::updateDescriptorSets(const SimulationState &simulationState) {
    ...
    Cmn::bindCombinedImageSampler(resources.device, colormap.view, colormap.sampler, descriptorSet, 1);
...
void Background2DPipeline::draw(vk::CommandBuffer &cb, const SimulationState &simulationState) {
    ...
    cb.pushConstants(pipelineLayout, vk::ShaderStageFlagBits::eAll, 0, sizeof(ParticlePushStruct), &pushStruct);
    cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayout, 0, 1, &descriptorPool.sets[0],
                          0, nullptr);
